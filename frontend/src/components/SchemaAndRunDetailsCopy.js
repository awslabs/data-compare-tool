        ...
        case POPULATE_TABLE:
            return {
                ...
                tableNamesToBeLoaded: data1.hostDetailsList.find(() => hostNameSelected)
                    ...
                    .find(schema => schema.value===action.schemaName).tableList
            }
...
function Nestedselect() {
    ...
    const handleOnSchemaRunClick = event =>{
        ...
        let tempArr=[];
        let slnumber=1;
        for(let i=0,len=data1.hostDetailsList.find(() => hostNameSelected)
            .databaseList.find(() => dbNameSelected).schemaList
            .find(schema => schema.value===schemaNameSelected).tableList.length;i<len;i++){
            for(let j=0,len1=data1.hostDetailsList.find(() => hostNameSelected)
                .databaseList.find(() => dbNameSelected).schemaList
                .find(schema => schema.value===schemaNameSelected).tableList[i].tableRun.length;j<len1;j++){
                let obj ={};
                obj.slNo=slnumber;
                slnumber++;
                ...
                obj.tableName=data1.hostDetailsList.find(() => hostNameSelected)
                    .databaseList.find(() => dbNameSelected).schemaList
                    .find(schema => schema.value===schemaNameSelected).tableList[i].tableName;
                obj.tableRun = data1.hostDetailsList.find(() => hostNameSelected)
                    .databaseList.find(() => dbNameSelected).schemaList
                    .find(schema => schema.value===schemaNameSelected).tableList[i].tableRun[j].run;

                obj.runDate = data1.hostDetailsList.find(() => hostNameSelected)
                    .databaseList.find(() => dbNameSelected).schemaList
                    .find(schema => schema.value===schemaNameSelected).tableList[i].tableRun[j].executionDate;
    ...
    const handleOnTableRunClick = event =>{
        ...
        for(let i=0,len=data1.hostDetailsList.find(() => hostNameSelected)
            .databaseList.find(() => dbNameSelected).schemaList.find(() => schemaNameSelected).
            tableList.find(() => tableNameSelected).tableRun.length;i<len;i++){
            let obj={};
            obj.slNo=slnumber;
            obj.tableName=data1.hostDetailsList.find(() => hostNameSelected)
                .databaseList.find(() => dbNameSelected).schemaList.find(() => schemaNameSelected).
